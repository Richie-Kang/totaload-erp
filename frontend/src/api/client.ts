...
async function fetchWithColdStartRetry(
  ...
  for (let i = 0; ; i++) {
    ...
    try {
      ...
      // Caller-aborted requests must not be retried.
    ...
    if (signal?.aborted) throw networkErr ?? new Error('aborted');
  }
...
export async function apiGet<T>(path: string, signal?: AbortSignal): Promise<T> {
  const res = await fetchWithColdStartRetry(() => fetch(path, { signal }), signal);
