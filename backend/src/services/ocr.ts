...
export const ALLOWED_OCR_PROVIDERS: ReadonlySet<OcrProvider> = new Set(['upstage', 'codex', 'gemini']);
...
export function failedResult(
  message = 'ocr-service 응답 없음',
  errorCode = 'OCR_UNAVAILABLE',
  provider: OcrProvider = DEFAULT_OCR_PROVIDER,
): ExtractResult {
  return { fields: emptyFields(), raw: null, status: 'failed', warnings: [message], errorCode, provider };
}
...
export async function extract(
  ...
  try {
    ...
    const respProvider = (data.provider as OcrProvider) ?? provider;
    return {
      ...
      provider: ALLOWED_OCR_PROVIDERS.has(respProvider) ? respProvider : provider,
    };
  } catch {
    return failedResult('ocr-service 응답 없음', 'OCR_UNAVAILABLE', provider);
  }
}
